...
- [ ] Удалить `app/logs/` (мёртвая папка, все логи → `logs/`)
...
- [ ] Провести QA по docs/operations/TESTCASES.md и зафиксировать результаты
...
- [x] Исключить повторное создание процессов `app/entrypoints/bot.py` (`dev_run_all`: lock + pre-kill + `--force`)

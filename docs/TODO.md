...
## Dashboard
_Актуально на 6 апреля 2026._

**Общий прогресс по backlog:** `45/86` (`52.3%`)

![Roadmap dashboard](assets/todo-dashboard.svg)

`Обновить графику:` `.\.venv\Scripts\python.exe scripts\render_todo_dashboard.py`

<details>
<summary>Табличная детализация</summary>

| Направление | Статус | Прогресс |
| --- | --- | --- |
| Текущий фокус MVP (апрель 2026) | ACTIVE | `0/3` (`0%`) |
| Этап 1 — Минимальные изменения, высокая стабильность | DONE | `6/6` (`100%`) |
| Этап 2 — Масштабируемость 1000+ | DONE | `5/5` (`100%`) |
| Этап 3 — Оптимизация затрат и качество распознавания | DONE | `8/8` (`100%`) |
| Этап 4 — Надежность и наблюдаемость | DONE | `8/8` (`100%`) |
| Этап 5 — UX бота | ACTIVE | `3/6` (`50%`) |
| Этап 6 — Хвосты из dialogue_dump | ACTIVE | `6/11` (`54.5%`) |
| Этап 7 — Альтернативный LLM-провайдер | PLANNED | `0/5` (`0%`) |
| Этап 8 — Омниканальный intake (B2B) | PLANNED | `0/5` (`0%`) |
| Этап 9 — Интеграции (МойСклад / 1С / iiko / r_keeper) | PLANNED | `0/8` (`0%`) |
| Этап 10 — Коммерциализация и удержание | PLANNED | `0/5` (`0%`) |
| Этап 11 — Процессное масштабирование | PLANNED | `0/7` (`0%`) |
| Этап 12 — Post-stage3 optimization backlog | DONE | `2/2` (`100%`) |
| Аудит веток и экспериментов | DONE | `7/7` (`100%`) |

</details>

## Ближайший фокус (next up)
1. Закрыть Этап 5: агрегировать split-альбомы, вернуть мягкую дедупликацию, добавить fixtures для smoke.
2. Закрыть Этап 6: довести HTTPS+webhook на VPS и `/status` для прозрачности очереди.
3. Зафиксировать базовый SLA MVP (время, качество, доля ручных доработок) и завести еженедельный контроль.
4. Подготовить старт Этапа 9: сначала связка `MAX + МойСклад + 1С` как базовый контур для РФ.
5. После стабилизации — запуск Этапов 8/10 через единый канал продаж и onboarding.

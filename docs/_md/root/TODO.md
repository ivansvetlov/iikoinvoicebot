...
## Этап 11 — Voice-to-Order и расширение на сервисные ниши
- [ ] Voice intake в MAX/Telegram: сотрудник отправляет голосовое «нужен расходник X», система распознаёт потребность и формирует заявку
- [ ] Zero-click подтверждение: бот возвращает «расходник заказан» + ETA + статус поставки
- [ ] Каталог расходников по ролям: мастер маникюра, барбер, косметолог, клининг, автосервис
- [ ] Ограничения и контроль: бюджетные лимиты, согласование старшим смены/владельцем
- [ ] Интеграция с закупкой: МойСклад/1С как backoffice для автозаказа и прихода
- [ ] Отдельная ветка для эксперимента: `feature/voice-to-order`
- [ ] Пилот 2-3 микро-вертикалей вне HoReCa (beauty, small retail, service teams)

...
class Msg:
    ...
    SYNC_NOM_CONFIRM = (
        ...
        "• сопоставляет позиции накладной с товарами iiko;\n"
        "• при необходимости создает новые товары и категории;\n"
        "• не выполняет оприходование и не меняет остатки.\n"
        "\n"
    ...
    POSTING_REVIEW_SUMMARY = "Готово к оприходованию: {ready}.\nТребует внимания: {blocked}."
    POSTING_REVIEW_ROW = "{status} {index}. {name} — {qty} {unit}"
    POSTING_REVIEW_MORE = "… и ещё {count} позиций."
    POSTING_REVIEW_UNITS = "Единицы измерения в iiko: {units}"
    POSTING_BLOCKED = (
        "Нельзя оприходовать: есть позиции, требующие внимания ({blocked}).\n"
        "Сначала синхронизируйте номенклатуру или исправьте единицы измерения."
    )

    BACK_CONFIRM = (
        "Вернуться к загрузке документов?\n"
        "Результат распознавания сохранится — вы сможете вернуться через /status."
    )
    BACK_CONFIRM_DONE = (
        "Можно снова отправлять документы.\n"
        "Результат распознавания сохранен — вернуться можно через /status."
    )

    SERVICE_MENU = (
        "Сервисные действия:\n"
        "• очистка склада удаляет остатки и товары (с подтверждением паролем);\n"
        "• откат доступен только после оприходования."
    )
    SERVICE_ROLLBACK_EMPTY = (
        "Оприходованные документы:\n"
        "Сейчас нет документов, доступных для отката."
    )
    SERVICE_CLEAR_STOCK_WARN = (
        "ВНИМАНИЕ: это полностью очистит склад и удалит товары.\n"
        "Введите пароль iiko для подтверждения."
    )
    ...
    BTN_INV_SYNC = "🔄 Синхронизировать номенклатуру"
    BTN_INV_SERVICE = "🛠 Сервис"
    BTN_SYNC_RUN = "▶️ Запустить синхронизацию"
    BTN_POST_CONFIRM = "✅ Подтвердить оприходование"
    BTN_REFRESH_UNITS = "🔄 Обновить ед. изм."
    BTN_RETRY = "🔁 Попробовать снова"
    BTN_YES_BACK = "✅ Да, вернуться"
    BTN_STAY = "Остаться"
    BTN_SERVICE_CLEAR_STOCK = "🧹 Очистить склад"
    BTN_SERVICE_ROLLBACK = "↩ Оприходованные документы"
